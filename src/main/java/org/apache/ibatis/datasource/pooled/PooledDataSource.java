    ...
    while (conn == null) {
      synchronized (state) {
        ...
        if (!state.idleConnections.isEmpty()) {
          // Pool has available connection，有连接可用
          conn = state.idleConnections.remove(0);
          if (log.isDebugEnabled()) {
            log.debug("Checked out connection " + conn.getRealHashCode() + " from pool.");
          }
        } else {
          // Pool does not have available connection,暂无空闲连接可用，但如果活跃连接数还未超出限制，继续创建连接
          if (state.activeConnections.size() < poolMaximumActiveConnections) {
            ...
            // Cannot create new connection，最大活跃连接列表已满，取出运行时长最长的连接
            ...
            if (longestCheckoutTime > poolMaximumCheckoutTime) {//超时
              ...
              try {
                ...
                //当前线程进入等待状态,在 pushConnection方法中会唤醒该对象
